...
export default class Day extends Component {
  ...
  dayCircleStyle = (isWeekend, isSelected, isToday,isLeaveStartDate,isLeaveEndDate, isInRange, event) => {
    const { customStyle } = this.props;
    const dayCircleStyle = [styles.dayCircleFiller, customStyle.dayCircleFiller];
    console.log('Day component',isLeaveStartDate,isLeaveEndDate);
    if (isSelected) {
      if (isToday) {
    ...
    if (isLeaveStartDate){
      dayCircleStyle.push(styles.startDayWrapper,customStyle.startDayWrapper);
    }

    if (isLeaveEndDate) {
      dayCircleStyle.push(styles.endDayWrapper,customStyle.endDayWrapper);
    }
    if (isInRange) {
      dayCircleStyle.push(styles.inRangeDay,customStyle.inRangeDay);
    }
    ...
    : (
      <TouchableOpacity onPress={this.props.onPress}>
        <View style={[styles.dayButton, customStyle.dayButton]}>
          <View style={this.dayCircleStyle(isWeekend, isSelected, isToday, isLeaveStartDate, isLeaveEndDate, isInRange, event)}>
